...
//Ks = 8 states * 3 bit per state = 24 bits. No Steganography makes 1 keys value = 24* 1 = 24
//kf = ks = 192
//kt = 8 bit per xt. No Steganography makes 1 keys value = 8* 1 = 8
// length = (24 + 24 +8)/8 = 7 words.
//total bits = 24+24+8=56
reg [2:0]key_storage_mem[6:0];
reg [6:0] keys_mem_address;
reg [55:0] key_storage_reg;  // key_storage_reg =[steganography(Ks) 24 bit, steganography(Kf) 24 bit, steganography(Kt) 8 bit] 
...
always@(*)
begin
 if (keys_mem_address == 7) //no. of words
		begin
			for(i=0; i<=6; i=i+1)
				begin
...
begin
	...
	if (receive_done == 1 && keys_mem_address <= 6)
		begin
			key_storage_mem[keys_mem_address] <= receive_data;
			keys_mem_address <= keys_mem_address+1;
		end
	else if (keys_mem_address == 7 && key_storage_valid == 1)
		begin
			Ks <= key_storage_reg[55:32]; //[Ks] 
			Kf <= key_storage_reg[31:8]; //[Kf]
			Kt <= key_storage_reg[7:0];    //[Kt]
			ks_kf_kt_valid<= 1;
		end
    
     if (keys_mem_address > 7)
    begin
        ip_stream_valid <= 1;
