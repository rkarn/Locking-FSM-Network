...
parameter steganography_keybit_length = 168;
parameter key_address_bitlength = 5;
//memory length calculation:
//Ks = 8 states * 3 bit per state = 24 bits. Steganography makes 3 keys value = 24* 2 = 72
//kf = ks = 72
//kt = 8 bit per xt. Steganography makes 3 keys value = 8* 3 = 24
// length = (72 + 72 + 24)/8 = 21 words.
//total bits = 72+72+24=168
reg [7:0]key_storage_mem[Num_words-1:0];
reg [key_address_bitlength-1:0] keys_mem_address;
reg [steganography_keybit_length-1:0] key_storage_reg;  // key_storage_reg =[steganography(Ks) 48 bit, steganography(Kf) 48 bit, steganography(Kt) 16 bit] 
...
always@(*)
begin
 if (keys_mem_address == Num_words)
		begin
			for(i=0; i<=Num_words-1; i=i+1)
				begin
...
begin
	...
	if (receive_done == 1 && keys_mem_address <= Num_words-1)
		begin
			key_storage_mem[keys_mem_address] <= receive_data;
			keys_mem_address <= keys_mem_address+1;
		end
	else if (keys_mem_address == Num_words && key_storage_valid == 1)
		begin
			Ks <= key_storage_reg[167:144]; //[Ks,..,..] 
			Kf <= key_storage_reg[47:24]; //[..,..,Kf]
			Kt <= key_storage_reg[23:16];    //[Kt,..,..]
			ks_kf_kt_valid<= 1;
		end
    
     if (keys_mem_address > Num_words-1)
    begin
        ip_stream_valid <= 1;
